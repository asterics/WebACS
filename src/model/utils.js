...
export function decodeForXml (string) {
...
export function encodeForXml(string) {
    string = decodeForXml(string); // if string already has encoded chars -> decode them, prevents double-encoding
	return string.replace(/&/g, '&amp;')
        .replace(/</g, '&lt;')
        .replace(/>/g, '&gt;')
        .replace(/"/g, '&quot;')
