 ...
 */
 ...
 import GuiViewElement from "./guiViewElement.js";
 import { gridStepType } from "../acsNamespace.js";
...
// ***********************************************************************************************************************
	var addComponent = function(comp, isNewComponent) {
		if (comp.gui) {
			if (isNewComponent && model.modelGui.getDecoration()) { // if decoration enabled, position componentGUI below
				comp.gui.setNewPosition({x: comp.gui.getX(), y: comp.gui.getY() + vConst.GUIVIEW_DECORATIONHEIGHT});
			}
			if (comp.gui.getIsExternal()) { // external components can be dragged and resized on the whole canvas
				componentGUIs.push({component: comp,
									guiViewElement: GuiViewElement(	model,
																		comp.gui,
																		guiLayer,
																		{width: vConst.GUIVIEWELEMENT_NORMSCREENRES_X - comp.gui.getX(), height: vConst.GUIVIEWELEMENT_NORMSCREENRES_Y - comp.gui.getY()},
																		{left: 0, right: vConst.GUIVIEWELEMENT_NORMSCREENRES_X, upper: 0, lower: vConst.GUIVIEWELEMENT_NORMSCREENRES_Y},
																		comp.getId(),
																		vConst.GUIVIEW_EXTERNALCOMPONENTGUIBACKGROUND,
																		editorProperties)});			
				...
				componentGUIs.push({component: comp,
									guiViewElement: GuiViewElement(	model,
																		...
																		comp.getId(),
																		vConst.GUIVIEW_COMPONENTGUIBACKGROUND,
																		editorProperties)});
	...
	var makeGrid = function() {
		...
		switch (editorProperties.getGridSteps()) {
			case gridStepType.SMALL: steps = vConst.GUIVIEW_GRIDSTEPS_SMALL;
										 break;
			case gridStepType.MEDIUM: steps = vConst.GUIVIEW_GRIDSTEPS_MEDIUM;
										  break;
			case gridStepType.LARGE: steps = vConst.GUIVIEW_GRIDSTEPS_LARGE;
										 break;
			case gridStepType.HUGE: steps = vConst.GUIVIEW_GRIDSTEPS_HUGE;
										break;											 
		}
		// draw vertical lines
		var linePos = steps;
		while (linePos < editorProperties.getGuiDesignerSize().width) {
			gridLines.push(new Kinetic.Line({
				...
				stroke: vConst.GUIVIEW_GRIDLINECOLOR,
				strokeWidth: 1
			}));
	...
	var screenResChangedEventHandler = function() {
		...
		switchGrid(editorProperties.getShowGrid());
		guiLayer.draw();		
		areGUI.setSizeBoundsMax({width: vConst.GUIVIEWELEMENT_NORMSCREENRES_X - model.modelGui.areGuiWindow.getX(), height: vConst.GUIVIEWELEMENT_NORMSCREENRES_Y - model.modelGui.areGuiWindow.getY()});
		areGUI.setDragBounds({left: 0, right: vConst.GUIVIEWELEMENT_NORMSCREENRES_X, upper: 0, lower: vConst.GUIVIEWELEMENT_NORMSCREENRES_Y});
		for (var i = 0; i < componentGUIs.length; i++) {
			if (componentGUIs[i].guiViewElement.getGui().getIsExternal()) {
				componentGUIs[i].guiViewElement.setSizeBoundsMax({width: vConst.GUIVIEWELEMENT_NORMSCREENRES_X - componentGUIs[i].guiViewElement.getX(), height: vConst.GUIVIEWELEMENT_NORMSCREENRES_Y - componentGUIs[i].guiViewElement.getY()});
				componentGUIs[i].guiViewElement.setDragBounds({left: 0, right: vConst.GUIVIEWELEMENT_NORMSCREENRES_X, upper: 0, lower: vConst.GUIVIEWELEMENT_NORMSCREENRES_Y});
			}
		}
	}
...
// ***********************************************************************************************************************
	...
	areGUI = GuiViewElement(model,
								model.modelGui.areGuiWindow,
								guiLayer, 
								{width: vConst.GUIVIEWELEMENT_NORMSCREENRES_X - model.modelGui.areGuiWindow.getX(), height: vConst.GUIVIEWELEMENT_NORMSCREENRES_Y - model.modelGui.areGuiWindow.getY()},
								{left: 0, right: vConst.GUIVIEWELEMENT_NORMSCREENRES_X, upper: 0, lower: vConst.GUIVIEWELEMENT_NORMSCREENRES_Y},
								vConst.GUIVIEW_AREWINDOWNAME,
								vConst.GUIVIEW_AREWINDOWBACKGROUND,
								editorProperties);
	areGUI.setDecoration(model.modelGui.getDecoration());
