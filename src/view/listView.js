 ...
 */
 ...
 import { componentType } from "../acsNamespace.js";
...
// ************************************************** static methods *****************************************************
// ***********************************************************************************************************************
	var makeCancelButton = function(model) {
...
// ************************************************** private variables **************************************************
// ***********************************************************************************************************************
	var sensorViewList = []; // Array<ListComponentView>
	var processorViewList = []; // Array<ListComponentView>
	var actuatorViewList = []; // Array<ListComponentView>
...
// ***********************************************************************************************************************
	var reloadListView = function() {
		...
		for (var i = 0; i < model.componentList.length; i++) {
			switch (model.componentList[i].getType()) {
				case componentType.SENSOR:	sensorViewList.push(ListComponentView(containerId, sensorList, model.componentList[i], model, returnObj));
												...
												break;
				case componentType.PROCESSOR:	processorViewList.push(ListComponentView(containerId, processorList, model.componentList[i], model, returnObj)); 
													...
													break;
				case componentType.ACTUATOR:	actuatorViewList.push(ListComponentView(containerId, actuatorList, model.componentList[i], model, returnObj)); 
	...
	var findListComponentView = function(comp) {
		var seekList = [];
		switch (comp.getType()) {
			case componentType.SENSOR: seekList = sensorViewList; break;
			case componentType.PROCESSOR: seekList = processorViewList; break;
			case componentType.ACTUATOR: seekList = actuatorViewList; break;			
		}
	...
	var componentAddedEventHandler = function() {
		switch (model.componentList[model.componentList.length - 1].getType()) {
			case componentType.SENSOR:		sensorViewList.push(ListComponentView(containerId, sensorList, model.componentList[model.componentList.length - 1], model, returnObj)); 
												sensorViewList[sensorViewList.length - 1].events.registerHandler('listComponentViewSelectedEvent', listComponentViewSelectedEventHandler);
												if (listKeyboardMode) {
												...
												break;
			case componentType.PROCESSOR:	processorViewList.push(ListComponentView(containerId, processorList, model.componentList[model.componentList.length - 1], model, returnObj)); 
												processorViewList[processorViewList.length - 1].events.registerHandler('listComponentViewSelectedEvent',listComponentViewSelectedEventHandler);
												if (listKeyboardMode) {
												...
												break;
			case componentType.ACTUATOR: 	actuatorViewList.push(ListComponentView(containerId, actuatorList, model.componentList[model.componentList.length - 1], model, returnObj));
												actuatorViewList[actuatorViewList.length - 1].events.registerHandler('listComponentViewSelectedEvent',listComponentViewSelectedEventHandler);
												if (listKeyboardMode) {
	...
	var dataChannelAddedEventHandler = function() {
		if (model.dataChannelList[model.dataChannelList.length - 1].getInputPort()) { // channel is already complete - must be an undo
			...
			$('#' + containerId + '_actionInfo').append(makeCancelButton(model));
		...
		if (typeof model.recentlyRemovedChannel.startComponent === 'undefined') { // if the recently removed channel is a dataChannel
			if (model.recentlyRemovedChannel.getInputPort()) { // if the removed channel was a completed channel
				...
				switch (model.recentlyRemovedChannel.getOutputPort().getParentComponent().getType()) {
					case componentType.SENSOR: removeDataChannelFromList(sensorViewList, model.recentlyRemovedChannel.getOutputPort().getParentComponent(), model.recentlyRemovedChannel, true); break;
					case componentType.PROCESSOR: removeDataChannelFromList(processorViewList, model.recentlyRemovedChannel.getOutputPort().getParentComponent(), model.recentlyRemovedChannel, true); break;
					case componentType.ACTUATOR: removeDataChannelFromList(actuatorViewList, model.recentlyRemovedChannel.getOutputPort().getParentComponent(), model.recentlyRemovedChannel, true); break;
				}
				// remove from inputPort
				switch (model.recentlyRemovedChannel.getInputPort().getParentComponent().getType()) {
					case componentType.SENSOR: removeDataChannelFromList(sensorViewList, model.recentlyRemovedChannel.getInputPort().getParentComponent(), model.recentlyRemovedChannel, false); break;
					case componentType.PROCESSOR: removeDataChannelFromList(processorViewList, model.recentlyRemovedChannel.getInputPort().getParentComponent(), model.recentlyRemovedChannel, false); break;
					case componentType.ACTUATOR: removeDataChannelFromList(actuatorViewList, model.recentlyRemovedChannel.getInputPort().getParentComponent(), model.recentlyRemovedChannel, false); break;
				}
		...
		if (typeof model.recentlyRemovedChannel.startComponent !== 'undefined') { // if the recently removed channel is an eventChannel	
			if (model.recentlyRemovedChannel.endComponent) { // if the removed channel was a completed channel
				...
				switch (model.recentlyRemovedChannel.startComponent.getType()) {
					case componentType.SENSOR: removeEventChannelFromList(sensorViewList, model.recentlyRemovedChannel.startComponent, model.recentlyRemovedChannel, true); break;
					case componentType.PROCESSOR: removeEventChannelFromList(processorViewList, model.recentlyRemovedChannel.startComponent, model.recentlyRemovedChannel, true); break;
					case componentType.ACTUATOR: removeEventChannelFromList(actuatorViewList,model.recentlyRemovedChannel.startComponent,  model.recentlyRemovedChannel, true); break;
				}
				// remove from inputPort
				switch (model.recentlyRemovedChannel.endComponent.getType()) {
					case componentType.SENSOR: removeEventChannelFromList(sensorViewList, model.recentlyRemovedChannel.endComponent, model.recentlyRemovedChannel, false); break;
					case componentType.PROCESSOR: removeEventChannelFromList(processorViewList, model.recentlyRemovedChannel.endComponent, model.recentlyRemovedChannel, false); break;
					case componentType.ACTUATOR: removeEventChannelFromList(actuatorViewList, model.recentlyRemovedChannel.endComponent, model.recentlyRemovedChannel, false); break;
				}
			} else { // an incomplete channel has been dropped
				$('#' + containerId + '_actionInfo').text('none');
				$('.' + containerId + '_dataInPortBtn').attr('disabled', '');
