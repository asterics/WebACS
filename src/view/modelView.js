 ...
 */
 ...
 import { portType } from "../acsNamespace.js";
 
 import ListView from "./listView.js";
 import GuiView from "./guiView.js";
 import ComponentView from "./componentView.js";
 import DataChannelView from "./dataChannelView.js";
 import EventChannelView from "./eventChannelView";
 import VisualAreaMarkerView from "../model/visualAreaMarker.js";

 import DragDropAction from "../model/dragDropAction.js";
...
// ************************************************** private variables **************************************************
// ***********************************************************************************************************************
	var modelTabPanel = TabPanel(modelContainerId, vConst.MODELVIEW_CLASSOFTAB, vConst.MODELVIEW_CLASSOFPANEL);
...
// ***********************************************************************************************************************
	var drawCompleteModel = function() {
		...
		for (i = 0; i < model.componentList.length; i++) {
			if (model.componentList[i].foundInComponentCollection) {
				componentViewList.push(ComponentView(model.componentList[i], model, returnObj, modelLayer));
		...
		for (i = 0; i < model.dataChannelList.length; i++) {
			dataChannelViewList[i] = DataChannelView(model.dataChannelList[i], model, returnObj, modelLayer);
		}
		for (i = 0; i < model.eventChannelList.length; i++) {
			eventChannelViewList[i] = EventChannelView(model.eventChannelList[i], model, returnObj, modelLayer);
		}
		for (i = 0; i < model.visualAreaMarkerList.length; i++) {
			visualAreaMarkerViewList[i] = VisualAreaMarkerView(model.visualAreaMarkerList[i], modelLayer);
		}
	...
	var getCloserComponent = function(closestComponent, otherComponent, centerOfThisComponent, centerOfOtherComponent) {
		var centerOfClosestComponent = {x: closestComponent.getX() + (vConst.COMPONENTVIEW_ELEMENTWIDTH / 2),
										y: closestComponent.getY() + (vConst.COMPONENTVIEW_ELEMENTHEIGHT / 2)}; // deliberately using center of standard-size component, ignoring different heights
	...
	var componentAddedEventHandler = function() {
		if (model.componentList.length > 0) componentViewList.push(ComponentView(model.componentList[model.componentList.length - 1], model, returnObj, modelLayer));
		modelLayer.draw();
	}
	...
	var dataChannelAddedEventHandler = function() {
		dataChannelViewList.push(DataChannelView(model.dataChannelList[model.dataChannelList.length -1], model, returnObj, modelLayer));
	...
	var eventChannelAddedEventHandler = function() {
		eventChannelViewList.push(EventChannelView(model.eventChannelList[model.eventChannelList.length -1], model, returnObj, modelLayer));
		// if the channel is currently being drawn and the user is using the keyboard, set a default endpoint to make unfinished channel visible to the user
...
// ***********************************************************************************************************************
	...
	returnObj.focusNextComponent = function(direction) {
		...
		if ((componentViewList.length > 1) && (model.selectedItemsList.length > 0) && (typeof model.selectedItemsList[0].inputPortList != 'undefined')) {
			var centerOfThisComponent = {x: model.selectedItemsList[0].getX() + (vConst.COMPONENTVIEW_ELEMENTWIDTH / 2),
										 y: model.selectedItemsList[0].getY() + (vConst.COMPONENTVIEW_ELEMENTHEIGHT / 2)}; // deliberately using center of standard-size component, ignoring different heights
			...
			for (var i = 0; i < componentViewList.length; i++) {
				centerOfOtherComponent = {x: componentViewList[i].getComponent().getX() + (vConst.COMPONENTVIEW_ELEMENTWIDTH / 2),
										  y: componentViewList[i].getComponent().getY() + (vConst.COMPONENTVIEW_ELEMENTHEIGHT / 2)}; // deliberately using center of standard-size component, ignoring different heights				
...
// ***********************************************************************************************************************
	...
	li1.textContent = vConst.MODELVIEW_MODELDESIGNERHEADER;
	...
	li2.textContent = vConst.MODELVIEW_GUIDESIGNERHEADER;
	...
	li3.textContent = vConst.MODELVIEW_LISTVIEWHEADER;
	...
	returnObj.selectedComponentsGroup = new Kinetic.Group({
		...
		dragBoundFunc: function(pos) {
			...
			if (this.rightBound > vConst.MODELVIEW_MODELDESIGNERSIZEX) {
				dx = dx - (this.rightBound - vConst.MODELVIEW_MODELDESIGNERSIZEX);
				this.rightBound = vConst.MODELVIEW_MODELDESIGNERSIZEX;
			}
			if (this.upperBound < 0) {
				dy = dy - this.upperBound;
				this.upperBound = 0;
			}
			if (this.lowerBound > vConst.MODELVIEW_MODELDESIGNERSIZEY) {
				dy = dy - (this.lowerBound - vConst.MODELVIEW_MODELDESIGNERSIZEY);
				this.lowerBound = vConst.MODELVIEW_MODELDESIGNERSIZEY;
			}
	...
	modelLayer.on('mousedown', function() {
		...
		if (!((model.dataChannelList.length > 0) && (!model.dataChannelList[model.dataChannelList.length - 1].getInputPort()))) {
			...
			focusRect = new Kinetic.Rect({ 
				...
				width: 0,
				height: 0,
				fill: vConst.MODELVIEW_FOCUSRECTCOLOR,
				stroke: vConst.MODELVIEW_FOCUSRECTSTROKECOLOR,
				strokeWidth: 1,
				listening: true
			});
	...
	returnObj.selectedComponentsGroup.on('dragstart', function() {
		dragging = true;
		dragAct = DragDropAction(model, getSelectedComponentsList());
	});
	...
	guiView = GuiView(model, 'guiPanel' + modelContainerId, editorProperties);
	listView = ListView('listPanel' + modelContainerId, model, clipBoard, 'listTab' + modelContainerId, modelTabPanel);
