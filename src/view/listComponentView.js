...
// ***********************************************************************************************************************
	...
	var generatePortList = function(id, portList) {
		...
		for (var i = 0; i < portList.length; i++) {
			...
			if (portList[i].getType() === portType.INPUT) {
				...
				$btn.click(function(port) {
					return function() {
						if (!channelExists(port)) {
							...
							$('#' + containerId + '_actionInfo').append(ListView().makeCancelButton(model));
...
// ***********************************************************************************************************************
	...
	returnObj.focusFirstChannel = function() {
		...
		} else if (completePortList[focussedPortIndex].getType() === portType.INPUT) {
			for (var i = 0; i < model.dataChannelList.length; i++) {
		...
		} else if (completePortList[focussedPortIndex].getType() === portType.OUTPUT) {
			for (var i = 0; i < model.dataChannelList.length; i++) {
