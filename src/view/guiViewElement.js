 ...
 */
 ...
 import GuiResizeAction from "../model/guiResizeAction.js";
 import GuiDragDropAction from "../model/guiDragDropAction.js";
...
// ***********************************************************************************************************************
	...
	var decorHeight;
	var controlWidth;
	var resizeAct = null; // GuiResizeAction
	var dragAct = null; // GuiDragDropAction
...
// ***********************************************************************************************************************
	...
	var xToScreenRes = function(x) { // takes a normalised x-coordinate and returns that coordinate mapped to the currently set resolution of the gui designer
		return x / vConst.GUIVIEWELEMENT_NORMSCREENRES_X * editorProperties.getGuiDesignerSize().width;
	}
	...
	var yToScreenRes = function(y) { // takes a normalised y-coordinate and returns that coordinate mapped to the currently set resolution of the gui designer
		return y / vConst.GUIVIEWELEMENT_NORMSCREENRES_Y * editorProperties.getGuiDesignerSize().height;
	}
	...
	var xToNormRes = function(x) { // takes an x-coordinate and returns that coordinate mapped to the normalised resolution that is used for storing coordinates
		return x / editorProperties.getGuiDesignerSize().width * vConst.GUIVIEWELEMENT_NORMSCREENRES_X;
	}
	...
	var yToNormRes = function(y) { // takes an y-coordinate and returns that coordinate mapped to the normalised resolution that is used for storing coordinates
		return y / editorProperties.getGuiDesignerSize().height * vConst.GUIVIEWELEMENT_NORMSCREENRES_Y;
	}	
	...
	var getGridStep = function() {
		switch (editorProperties.getGridSteps()) {
			case gridStepType.SMALL: return vConst.GUIVIEW_GRIDSTEPS_SMALL;
										 break;
			case gridStepType.MEDIUM: return vConst.GUIVIEW_GRIDSTEPS_MEDIUM;
										  break;
			case gridStepType.LARGE: return vConst.GUIVIEW_GRIDSTEPS_LARGE;
										 break;
			case gridStepType.HUGE: return vConst.GUIVIEW_GRIDSTEPS_HUGE;
										break;											 
		}		
	}
...
// ***********************************************************************************************************************
	...
	returnObj.setDecoration = function(active) { // bool
		if (active) {
			if (decoration) {
				...
				img.src = vConst.GUIVIEWELEMENT_AREWINDOWDECOIMAGE;
			}
		} else {
	...
	returnObj.setAREControls = function(active) { // bool
		if (controls) {
			...
			var controlRect = new Kinetic.Rect({x: mainRect.getAbsolutePosition().x + mainRect.width() - controlWidth,
												...
												fill: vConst.GUIVIEWELEMENT_CONTROLSCOLOR});
			var controlText = new Kinetic.Text({x: controlRect.getX(),
												y: controlRect.getY() + controlRect.height()/2 - 5,
												width: controlWidth,
												text: vConst.GUIVIEWELEMENT_CONTROLSNAME,
												fontSize: vConst.GUIVIEWELEMENT_CONTROLSFONTSIZE,
												wrap: 'char',
												align: 'center',
												fill: vConst.GUIVIEWELEMENT_NAMECOLOR});
	...
	returnObj.setFocus = function(focus) { // boolean
		hasFocus = focus;
		if (focus) {
			mainRect.fill(vConst.GUIVIEWELEMENT_FOCUSCOLOR);
		} else {
	...
	returnObj.resize = function(direction) {
		resizeAct = GuiResizeAction(model, gui);
		var moveStep = 1;
	...
	returnObj.reposition = function(direction) {
		...
		dragAct = GuiDragDropAction(model, guis);
		var moveStep = 1;
		if (editorProperties.getEnableGrid()) moveStep = getGridStep();
...
// ***********************************************************************************************************************
	...
	decorHeight = Math.round(yToScreenRes(vConst.GUIVIEW_DECORATIONHEIGHT));
	controlWidth = Math.round(xToScreenRes(vConst.GUIVIEW_CONTROLSWIDTH));
	...
	nameText = new Kinetic.Text({x: xToScreenRes(gui.getX()),
								 ...
								 text: name,
								 fontSize: vConst.GUIVIEWELEMENT_FONTSIZE,
								 wrap: 'char',
								 align: 'center',
								 fill: vConst.GUIVIEWELEMENT_NAMECOLOR});
	...
	group = new Kinetic.Group({	draggable: true,
								dragBoundFunc: function(pos) {
									...
									// check the bounds
									var actBounds = {left: (dragBounds.left - dragOffsetX), right: (dragBounds.right - dragOffsetX), upper: (dragBounds.upper - dragOffsetY), lower: (dragBounds.lower - dragOffsetY)};
