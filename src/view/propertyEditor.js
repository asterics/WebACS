...
export default function (modelList, // ModelList
	...
	// ************************************************** private variables **************************************************
	// ***********************************************************************************************************************
	var propertiesTabPanel = TabPanel(vConst.PROPERTYEDITOR_MOTHERPANEL, vConst.PROPERTYEDITOR_CLASSOFTAB, vConst.PROPERTYEDITOR_CLASSOFPANEL);
	...
	var generatePropertiesForComponent = function () {
		...
		for (var h = 0; h < actModel.componentList[selectedElement].propertyList.length; h++) {
			...
			if (comboValues === '' && typetemp === dataType.BOOLEAN) {
				cell = row[h + 1].insertCell(1);
				var boolInput = null;
			...
			if (comboValues === '' && typetemp === dataType.INTEGER) {
				cell = row[h + 1].insertCell(1);
				numberInput = null;
			...
			if (comboValues === '' && typetemp === dataType.DOUBLE) {
				cell = row[h + 1].insertCell(1);
				numberInput = null;
				numberInput = document.createElement("INPUT");
			...
			if (comboValues === '' && typetemp === dataType.STRING) {
				cell = row[h + 1].insertCell(1);
				textInput = null;
				textInput = document.createElement("INPUT");
				textInput.setAttribute("type", "text");
				textInput.setAttribute("name", propName);
				textInput.setAttribute("value", decodeForXml(valtemp));
	...
	var generateInputPortsForComponent = function () {
		...
		for (var h = 0; h < actModel.componentList[selectedElement].inputPortList.length; h++) {
			...
			cell = row[h + 1].insertCell(1);
			cell.innerHTML = stringOfEnum(dataType, tempStringa);
	...
	var writeChannel = function (evt) {
		...
		if (t !== '---' && previousDropDownEntry === '---') {
			...
			dropdownList = null;
			dropdownList = document.createElement('select');
			for (var l = 0; l < listenerComponent.triggerEventList.length + 1; l++) {
				if (l === 0) {
					dropdownList.appendChild(new Option('---', l));
				} else {
	...
	li1.textContent = vConst.PROPERTYEDITOR_PROPERTIESHEADER;
	document.getElementById(vConst.PROPERTYEDITOR_TABLIST).appendChild(li1);
	...
	document.getElementById(vConst.PROPERTYEDITOR_MOTHERPANEL).appendChild(div);
	...
	li2.textContent = vConst.PROPERTYEDITOR_INPUTHEADER;
	document.getElementById(vConst.PROPERTYEDITOR_TABLIST).appendChild(li2);
	...
	li3.textContent = vConst.PROPERTYEDITOR_OUTPUTHEADER;
	document.getElementById(vConst.PROPERTYEDITOR_TABLIST).appendChild(li3);
	div = document.createElement('div');
	...
	li4.textContent = vConst.PROPERTYEDITOR_TRIGGERHEADER;
	document.getElementById(vConst.PROPERTYEDITOR_TABLIST).appendChild(li4);
	div = document.createElement('div');
	...
	li5.textContent = vConst.PROPERTYEDITOR_LISTENERHEADER;
	document.getElementById(vConst.PROPERTYEDITOR_TABLIST).appendChild(li5);
	div = document.createElement('div');
	...
	li6.textContent = vConst.PROPERTYEDITOR_EVENTHEADER;
	document.getElementById(vConst.PROPERTYEDITOR_TABLIST).appendChild(li6);
	div = document.createElement('div');
	...
	document.getElementById(vConst.PROPERTYEDITOR_MOTHERPANEL).appendChild(divAlert);
