...
// ***********************************************************************************************************************
	...
	var selectLine = function() {
		returnObj.line.stroke(vConst.EVENTCHANNELVIEW_SELECTEDSTROKECOLOR);
		returnObj.line.dashEnabled(true);
	}
	
	// ********************************************** handlers ***********************************************************
	var componentPositionChangedEventHandlerStartComponent = function() {
			returnObj.line.points([	ec.startComponent.getX() + vConst.EVENTCHANNELVIEW_TRIGGERPOSX,
									ec.startComponent.getY() + getComponentHeight(ec.startComponent) + vConst.EVENTCHANNELVIEW_TRIGGERBELOWCOMPONENT, 
	...
	var componentPositionChangedEventHandlerEndComponent = function() {
			returnObj.line.points([	returnObj.line.points()[0], 
									...
									ec.endComponent.getX() + vConst.EVENTCHANNELVIEW_LISTENERPOSX,
									ec.endComponent.getY() + getComponentHeight(ec.endComponent) + vConst.EVENTCHANNELVIEW_LISTENERBELOWCOMPONENT]);
		if (!modelView.isDragging()) {
			modelLayer.draw();
		}									
	}
	...
	var deSelectedEventHandler = function() {
		returnObj.line.stroke(vConst.EVENTCHANNELVIEW_STROKECOLOR);
...
// ************************************************** public stuff *******************************************************
// ***********************************************************************************************************************
	var returnObj = ChannelView(model, modelView, modelLayer);
...
// ***********************************************************************************************************************
	...
	if (ec.endComponent) { // i.e. channel is already complete
		...
		returnObj.line.points([	ec.startComponent.getX() + vConst.EVENTCHANNELVIEW_TRIGGERPOSX,
								ec.startComponent.getY() + getComponentHeight(ec.startComponent) + vConst.EVENTCHANNELVIEW_TRIGGERBELOWCOMPONENT,
								ec.startComponent.getX() + vConst.EVENTCHANNELVIEW_TRIGGERPOSX,
								ec.startComponent.getY() + getComponentHeight(ec.startComponent) + vConst.EVENTCHANNELVIEW_TRIGGERBELOWCOMPONENT]);
	}
	
	// highlight channel when mouse is over hitGraph
	returnObj.line.on('mouseover', function(e) {
		returnObj.line.strokeWidth(vConst.CHANNELVIEW_STROKEWIDTH+2);
		modelLayer.draw();
	});
	returnObj.line.on('mouseout', function(e) {
		if (returnObj.line) { // mouseout will fire if channel has already been deleted, when mouse was over the channel at the time of deleting it
			returnObj.line.strokeWidth(vConst.CHANNELVIEW_STROKEWIDTH);
			modelLayer.draw();
		}
	});	
