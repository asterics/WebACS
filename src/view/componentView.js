...
// ***********************************************************************************************************************
	var visible = true;
	var view = null;
	var elementHeight = vConst.COMPONENTVIEW_ELEMENTHEIGHT;
...
// ***********************************************************************************************************************
	...
	var buildView = function() {
		...
		if ((component.outputPortList.length > 3) || (component.inputPortList.length > 3)) {
			if (component.outputPortList.length > component.inputPortList.length) {
				elementHeight = elementHeight + (component.outputPortList.length-3) * vConst.COMPONENTVIEW_PORTHEIGHTPLUSGAP;
			} else {
				elementHeight = elementHeight + (component.inputPortList.length-3) * vConst.COMPONENTVIEW_PORTHEIGHTPLUSGAP;
			}
		}	
		...
		var mainRect = new Kinetic.Rect({ 
			x: component.getX(),
			y: component.getY(),
			width: vConst.COMPONENTVIEW_ELEMENTWIDTH,
			height: elementHeight,
			fill: vConst.COMPONENTVIEW_COMPONENTCOLOR,
			stroke: vConst.COMPONENTVIEW_STROKECOLOR,
			strokeWidth: 1,
			cornerRadius: 5,
			listening: true
		});
		...
		var topRect = new Kinetic.Rect({
			...
			y: component.getY(),
			width: vConst.COMPONENTVIEW_ELEMENTWIDTH,
			height: vConst.COMPONENTVIEW_TOPRECTHEIGHT,
			fill: vConst.COMPONENTVIEW_COMPONENTHEADERCOLOR,
			stroke: vConst.COMPONENTVIEW_STROKECOLOR,
			strokeWidth: 1,
			cornerRadius: 5,
			listening: false
		...
		var headerText = new Kinetic.Text({
			x: component.getX() + vConst.COMPONENTVIEW_HEADERTEXTPOSITIONX,
			y: component.getY() + vConst.COMPONENTVIEW_HEADERTEXTPOSITIONY,
			text: component.getId(),
			fontSize: vConst.COMPONENTVIEW_FONTSIZE,
			fill: vConst.COMPONENTVIEW_TEXTCOLOR,
			width: vConst.COMPONENTVIEW_HEADERTEXTWIDTH,
			wrap: 'char'
		});
		...
		for (var i = 0; i < component.inputPortList.length; i++) {
			...
			inputPortViewList[i]['port'] = new Kinetic.Rect({
				x: component.getX() - vConst.COMPONENTVIEW_INPUTPORTLEFTOFCOMPONENT,
				y: component.getY() + vConst.COMPONENTVIEW_FIRSTINPUTPORTY + vConst.COMPONENTVIEW_PORTHEIGHTPLUSGAP * i,
				width: vConst.COMPONENTVIEW_PORTWIDTH,
				height: vConst.COMPONENTVIEW_PORTHEIGHT,
				fill: vConst.COMPONENTVIEW_INPUTPORTCOLOR,
				stroke: vConst.COMPONENTVIEW_STROKECOLOR,
				strokeWidth: 1,
				cornerRadius: 3,
				listening: true,
			...
			inputPortViewList[i]['label'] = new Kinetic.Text({
				x: component.getX() + vConst.COMPONENTVIEW_INPUTPORTLABELPOSITIONX,
				y: component.getY() + vConst.COMPONENTVIEW_FIRSTINPUTPORTY + 1 + vConst.COMPONENTVIEW_PORTHEIGHTPLUSGAP * i,
				text: component.inputPortList[i].getId(),
				fontSize: vConst.COMPONENTVIEW_FONTSIZE,
				fill: vConst.COMPONENTVIEW_TEXTCOLOR,
				width: vConst.COMPONENTVIEW_PORTLABELWIDTH
			});
		...
		for (var i = 0; i < component.outputPortList.length; i++) {
			...
			outputPortViewList[i]['port'] = new Kinetic.Rect({
				x: component.getX() + vConst.COMPONENTVIEW_OUTPUTPORTPOSITIONX,
				y: component.getY() + vConst.COMPONENTVIEW_FIRSTOUTPUTPORTPOSITIONY + vConst.COMPONENTVIEW_PORTHEIGHTPLUSGAP * i,
				width: vConst.COMPONENTVIEW_PORTWIDTH,
				height: vConst.COMPONENTVIEW_PORTHEIGHT,
				fill: vConst.COMPONENTVIEW_OUTPUTPORTCOLOR,
				stroke: vConst.COMPONENTVIEW_STROKECOLOR,
				strokeWidth: 1,
				cornerRadius: 3,
				listening: true
			...
			outputPortViewList[i]['label'] = new Kinetic.Text({
				x: component.getX() + vConst.COMPONENTVIEW_OUTPUTPORTLABELPOSITIONX,
				y: component.getY() + vConst.COMPONENTVIEW_FIRSTOUTPUTPORTPOSITIONY + 1 + vConst.COMPONENTVIEW_PORTHEIGHTPLUSGAP * i,
		...
		if (component.listenEventList.length > 0) {
			eventInPortView = new Kinetic.Shape({
				x: component.getX() + vConst.COMPONENTVIEW_EVENTLISTENERPORTPOSITIONX,
				y: component.getY() + elementHeight - vConst.COMPONENTVIEW_EVENTPORTYINSIDECOMPONENT,
				fill: vConst.COMPONENTVIEW_EVENTINPUTPORTCOLOR,
				stroke: vConst.COMPONENTVIEW_STROKECOLOR,
				strokeWidth: 1,
				drawFunc: function(context) {
					context.beginPath();
					context.moveTo(0, 0);
					context.lineTo(0, vConst.COMPONENTVIEW_EVENTPORTHEIGHT);
					context.lineTo(vConst.COMPONENTVIEW_EVENTPORTWIDTH / 2, vConst.COMPONENTVIEW_EVENTPORTHEIGHT * 0.8);
					context.lineTo(vConst.COMPONENTVIEW_EVENTPORTWIDTH, vConst.COMPONENTVIEW_EVENTPORTHEIGHT);
					context.lineTo(vConst.COMPONENTVIEW_EVENTPORTWIDTH, 0);
					context.lineTo(0, 0);
					context.closePath();
		...
		if (component.triggerEventList.length > 0) {
			eventOutPortView = new Kinetic.Shape({
				x: component.getX() + vConst.COMPONENTVIEW_EVENTTRIGGERPORTPOSITIONX,
				y: component.getY()+elementHeight - vConst.COMPONENTVIEW_EVENTPORTYINSIDECOMPONENT,
				fill: vConst.COMPONENTVIEW_EVENTOUTPUTPORTCOLOR,
				stroke: vConst.COMPONENTVIEW_STROKECOLOR,
				strokeWidth: 1,
				drawFunc: function(context) {
					context.beginPath();
					context.moveTo(0, 0);
					context.lineTo(0, vConst.COMPONENTVIEW_EVENTPORTHEIGHT * 0.8);
					context.lineTo(vConst.COMPONENTVIEW_EVENTPORTWIDTH / 2, vConst.COMPONENTVIEW_EVENTPORTHEIGHT);
					context.lineTo(vConst.COMPONENTVIEW_EVENTPORTWIDTH, vConst.COMPONENTVIEW_EVENTPORTHEIGHT * 0.8);
					context.lineTo(vConst.COMPONENTVIEW_EVENTPORTWIDTH, 0);
					context.lineTo(0, 0);
					context.closePath();
					context.fillStrokeShape(this);
		...
		selectedRect = new Kinetic.Rect({
			x: component.getX() - vConst.COMPONENTVIEW_SELECTIONFRAMEWIDTH,
			y: component.getY() - vConst.COMPONENTVIEW_SELECTIONFRAMEWIDTH,
			width: vConst.COMPONENTVIEW_ELEMENTWIDTH + (2 * vConst.COMPONENTVIEW_SELECTIONFRAMEWIDTH),
			height: elementHeight + (2 * vConst.COMPONENTVIEW_SELECTIONFRAMEWIDTH),
			fill: vConst.COMPONENTVIEW_SELECTIONFRAMECOLOR,
			stroke: vConst.COMPONENTVIEW_SELECTIONFRAMESTROKECOLOR,
			strokeWidth: 1,
			cornerRadius: 0,
			listening: true
		});
		...
		var errorRect = new Kinetic.Rect({ 
			x: component.getX() - vConst.COMPONENTVIEW_ERRORMARKERWIDTH,
			y: component.getY() - vConst.COMPONENTVIEW_ERRORMARKERWIDTH,
			width: vConst.COMPONENTVIEW_ELEMENTWIDTH + (2 * vConst.COMPONENTVIEW_ERRORMARKERWIDTH),
			height: elementHeight + (2 * vConst.COMPONENTVIEW_ERRORMARKERWIDTH),
			fill: vConst.COMPONENTVIEW_ERRORMARKERCOLOR,
			stroke: vConst.COMPONENTVIEW_ERRORMARKERCOLOR,
			strokeWidth: 1,
			cornerRadius: 0,
			listening: true
		});
	...
	var setViewPosition = function() {
		...
		ch[0].x(component.getX() - vConst.COMPONENTVIEW_ERRORMARKERWIDTH); // errorRect
		ch[0].y(component.getY() - vConst.COMPONENTVIEW_ERRORMARKERWIDTH); // errorRect
		ch[1].x(component.getX() - vConst.COMPONENTVIEW_SELECTIONFRAMEWIDTH); // selectedRect
		ch[1].y(component.getY() - vConst.COMPONENTVIEW_SELECTIONFRAMEWIDTH); // selectedRect
		ch[2].x(component.getX()); // mainRect
		ch[2].y(component.getY()); // mainRect
		ch[3].x(component.getX()); // topRect
		ch[3].y(component.getY()); // topRect
		ch[4].x(component.getX() + vConst.COMPONENTVIEW_HEADERTEXTPOSITIONX); // headerText
		ch[4].y(component.getY() + vConst.COMPONENTVIEW_HEADERTEXTPOSITIONY); // headerText
		for (var i = 0; i < inputPortViewList.length; i++) {
			inputPortViewList[i]['port'].x(component.getX() - vConst.COMPONENTVIEW_INPUTPORTLEFTOFCOMPONENT);
			inputPortViewList[i]['port'].y(component.getY() + vConst.COMPONENTVIEW_FIRSTINPUTPORTY + vConst.COMPONENTVIEW_PORTHEIGHTPLUSGAP * i);
			inputPortViewList[i]['label'].x(component.getX() + vConst.COMPONENTVIEW_INPUTPORTLABELPOSITIONX);
			inputPortViewList[i]['label'].y(component.getY() + vConst.COMPONENTVIEW_FIRSTINPUTPORTY + 1 + vConst.COMPONENTVIEW_PORTHEIGHTPLUSGAP * i);
		}
		for (var i = 0; i < outputPortViewList.length; i++) {
			outputPortViewList[i]['port'].x(component.getX() + vConst.COMPONENTVIEW_OUTPUTPORTPOSITIONX);
			outputPortViewList[i]['port'].y(component.getY() + vConst.COMPONENTVIEW_FIRSTOUTPUTPORTPOSITIONY + vConst.COMPONENTVIEW_PORTHEIGHTPLUSGAP * i);
			outputPortViewList[i]['label'].x(component.getX() + vConst.COMPONENTVIEW_OUTPUTPORTLABELPOSITIONX);
			outputPortViewList[i]['label'].y(component.getY() + vConst.COMPONENTVIEW_FIRSTOUTPUTPORTPOSITIONY + 1 + vConst.COMPONENTVIEW_PORTHEIGHTPLUSGAP * i);
		}	
		if (eventInPortView) {
			eventInPortView.x(component.getX() + vConst.COMPONENTVIEW_EVENTLISTENERPORTPOSITIONX);
			eventInPortView.y(component.getY() + elementHeight - vConst.COMPONENTVIEW_EVENTPORTYINSIDECOMPONENT);
		}
		if (eventOutPortView) {
			eventOutPortView.x(component.getX() + vConst.COMPONENTVIEW_EVENTTRIGGERPORTPOSITIONX);
			eventOutPortView.y(component.getY() + elementHeight - vConst.COMPONENTVIEW_EVENTPORTYINSIDECOMPONENT);
		}
	}
	...
	var setSelectionBounds = function() {
		if (modelView.selectedComponentsGroup.children.length > 0) {
			...
			for (i = 0; i < modelView.selectedComponentsGroup.children.length; i++) {
				...
				right = modelView.selectedComponentsGroup.children[i].children[2].x() + vConst.COMPONENTVIEW_ELEMENTWIDTH;
				upper = modelView.selectedComponentsGroup.children[i].children[2].y();
				lower = modelView.selectedComponentsGroup.children[i].children[2].y() + vConst.COMPONENTVIEW_ELEMENTHEIGHT;
	...
	var updatePortFocusHighlighting = function() {
		for (var i = 0; i < inputPortViewList.length; i++) {
			if (portFocus.inP === i) {
				inputPortViewList[i].port.fill(vConst.COMPONENTVIEW_SELECTEDPORTCOLOR);
			} else {
				inputPortViewList[i].port.fill(vConst.COMPONENTVIEW_INPUTPORTCOLOR);
			}
		}
		for (var i = 0; i < outputPortViewList.length; i++) {
			if (portFocus.outP === i) {
				outputPortViewList[i].port.fill(vConst.COMPONENTVIEW_SELECTEDPORTCOLOR);
			} else {
				outputPortViewList[i].port.fill(vConst.COMPONENTVIEW_OUTPUTPORTCOLOR);
			}
		}
		if (eventInPortView) {
			if (portFocus.inEP > -1) {
				eventInPortView.fill(vConst.COMPONENTVIEW_SELECTEDPORTCOLOR);
			} else {
				eventInPortView.fill(vConst.COMPONENTVIEW_EVENTINPUTPORTCOLOR);
			}
		}
		if (eventOutPortView) {
			if (portFocus.outEP > -1) {
				eventOutPortView.fill(vConst.COMPONENTVIEW_SELECTEDPORTCOLOR);
			} else {
				eventOutPortView.fill(vConst.COMPONENTVIEW_EVENTOUTPUTPORTCOLOR);
			} 
		}
		modelLayer.draw();
	}
...
// ***********************************************************************************************************************
	...
	returnObj.setPortMode = function(newMode) {
		if (newMode) {
			selectedRect.fill(vConst.COMPONENTVIEW_SELECTIONFRAMECOLORPORTMODE);
			modelLayer.draw();
			focusFirstPort();
		} else {
			selectedRect.fill(vConst.COMPONENTVIEW_SELECTIONFRAMECOLOR);
			modelLayer.draw();
			unFocusAllPorts();
		}
		portMode = newMode;
